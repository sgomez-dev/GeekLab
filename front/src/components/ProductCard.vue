<template>
  <div
    v-if="!removed"
    class="product-card"
    :class="{ 'out-of-stock': isOutOfStock, 'admin-enabled': isAdmin }"
  >
    <div v-if="isOutOfStock" class="out-of-stock-overlay">
      <div class="overlay-content">
        ...
        <div v-if="isAdmin" class="overlay-actions">
          <button class="btn-small add" @click="openAddStockModal">
            Agregar stock
          </button>
          <button
    ...
    <div class="product-info">
      ...
      <div class="product-rating" v-if="product.numReviews >= 0">
        ...
        <span class="product-rating-text" v-if="product.numReviews > 0">{{
          (product.averageRating || 0).toFixed(1)
        }}</span>
      </div>
      <p class="product-stock" :class="{ 'low-stock': (localStock ?? 0) <= 5 }">
        Stock: {{ localStock ?? 0 }}
      </p>
      <button
        class="add-to-cart"
    ...
    <div class="modal">
      ...
      <div class="modal-actions">
        ...
        <button
          ...
          :disabled="isAddingStock || !addStockQty || addStockQty <= 0"
        >
          {{ isAddingStock ? "Guardando..." : "Añadir" }}
        </button>
      </div>
    </div>
  </div>
